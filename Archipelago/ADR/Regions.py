...
starting_regions = {
    "requires": [],
    "connects_to": ["FirstDimensionboost"],
    "starting": "True"
}
...
regionMap["AntimatterDimensions"] = {
    "requires": [],
    "connects_to": ["FirstDimensionboost"],
    "starting": "True"
}
...
def create_regions(world: World, multiworld: MultiWorld, player: int):
    ...
    for region in regionMap:
        ...
        if "starting" not in regionMap[region]:
            regionMap[region]["starting"] = "false"
    ...
    menu = create_region(world, multiworld, player, "Menu", None, ["AntimatterDimensions"])
    multiworld.regions += [menu]
    menuConn = multiworld.get_entrance("MenuToAntimatterDimensions", player)
    menuConn.connect(multiworld.get_region("AntimatterDimensions", player))
...
def create_events(world: World, multiworld: MultiWorld, player: int):
    for name, event in world.event_name_to_event.items():
        region = multiworld.get_region(event.get("region", "AntimatterDimensions"), player)
