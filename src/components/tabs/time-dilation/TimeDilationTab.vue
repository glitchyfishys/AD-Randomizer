...
export default {
  ...
  computed: {
    rebuyables() {
      return [
        DilationUpgrade.all[0],
        DilationUpgrade.all[1],
        DilationUpgrade.all[2]
      ];
    },
    upgrades() {
      return [
        [
          DilationUpgrade.all[6],
          DilationUpgrade.all[7],
          DilationUpgrade.all[8]
        ],
        [
          DilationUpgrade.all[9],
          DilationUpgrade.all[10],
          DilationUpgrade.all[11]
        ],
      ];
    },
    ...
    pelleRebuyables() {
      return [
        DilationUpgrade.all[3],
        DilationUpgrade.all[4],
        DilationUpgrade.all[5]
      ];
    },
    pelleUpgrades() {
      return [
        DilationUpgrade.all[13],
        DilationUpgrade.all[14]
      ];
    },
    ttGenerator() {
      return DilationUpgrade.all[12];
    },
    baseGalaxyText() {
  ...
  methods: {
    update() {
      ...
      if (this.baseGalaxies < 500 && DilationUpgrade[3].isBought) {
        this.tachyonGalaxyGain = DilationUpgrade[3].effectValue;
      } else {
        this.tachyonGalaxyGain = 1;
      }
      this.tachyonGalaxyGain *= DilationUpgrade[11].effectValue;
