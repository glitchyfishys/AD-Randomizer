...
export default {
  ...
  computed: {
    ...
    pauseTime(){
      return TimeSpan.fromMilliseconds(this.traps.pause).toStringShort();
    },
    dilationTime(){
      return TimeSpan.fromMilliseconds(this.traps.dilation).toStringShort();
    }
  },
  methods: {
    update() {
      this.traps.dilation = player.archipelago.dilationTrapTime;
      this.traps.pause = player.archipelago.pauseTrapTime;
...
<template>
  <div>
    ...
    <div v-if="traps.dilation > 0">
      Antimatter Dimensions are dilated for {{ dilationTime }}
    </div>
