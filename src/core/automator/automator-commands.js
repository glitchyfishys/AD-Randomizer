...
export const AutomatorCommands = [
  {
    ...
    validate: (ctx, V) => {
      ...
      if (ctx.PrestigeEvent[0].tokenType === T.Infinity) {
        ...
        if (advSetting && !EternityMilestone.all[3].isReached) {
          V.addError((ctx.duration || ctx.xHighest)[0],
            "Advanced Infinity autobuyer settings are not unlocked",
            `Reach ${quantifyInt("Eternity", EternityMilestone.all[3].config.eternities)}
            to use this command`);
          return false;
        }
      }
      if (ctx.PrestigeEvent[0].tokenType === T.Eternity) {
        if (!EternityMilestone.all[24].isReached) {
          V.addError(ctx.PrestigeEvent, "Eternity autobuyer is not unlocked",
            `Reach ${quantifyInt("Eternity", EternityMilestone.all[24].config.eternities)}
            to use this command`);
          return false;
        }
  ...
  },
  {
    ...
    validate: (ctx, V) => {
      ...
      if (ctx.PrestigeEvent && ctx.PrestigeEvent[0].tokenType === T.Eternity &&
        !EternityMilestone.all[24].isReached) {
        V.addError(ctx.PrestigeEvent, "Eternity autobuyer is not unlocked",
          `Reach ${quantifyInt("Eternity", EternityMilestone.all[24].config.eternities)}
          to use this command`);
        return false;
    ...
    compile: ctx => {
      ...
      return () => {
        ...
        const missingStudyCount = imported.purchasedStudies
          .filter(s => !GameCache.currentStudyTree.value.purchasedStudies.some(x => x.id == s.id)).length;
