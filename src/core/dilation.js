...
export function buyDilationUpgrade(id, bulk = 1) {
  ...
  const upgrade = DilationUpgrade[id - 1];
  if (id > 3 && id < 11) {
...
export function getTachyonGalaxyMult(thresholdUpgrade) {
  ...
  const upgrade = thresholdUpgrade === undefined ? DilationUpgrade[1].effectValue : thresholdUpgrade;
  ...
  const power = DilationUpgrade[13].canBeApplied
    ? DilationUpgrade[13].effectValue : 1;
...
export function getDilationGainPerSecond() {
  if (Pelle.isDoomed) {
    ...
    return new Decimal(tachyonEffect)
      .timesEffectsOf(DilationUpgrade[0], DilationUpgrade[10], DilationUpgrade[14])
...
export function tachyonGainMultiplier() {
  if (Pelle.isDisabled("tpMults")) return new Decimal(5);
  ...
  return DC.D1.timesEffectsOf(
    ...
    RealityUpgrade(8),
    RealityUpgrade(15)
  ).mul(Archipelago.tachyonParticles).pow(pow);
}
...
export function dilatedValueOf(value) {
  ...
  const dilationPenalty = 0.75 * Effects.product(DilationUpgrade[8]);
