import { GameMechanicState, BitPurchasableMechanicState } from "./game-mechanics";
...
export class InfinityUpgradeState extends BitPurchasableMechanicState {
  constructor(config) {
    super(config);
    if (config.charged) {
  ...
  set bits(value) {
    return player.infinityUpgrades = value;
  }
  ...
  purchase() {
    if (super.purchase()) {
      ...
      if (this.config.id > 11) {
        if (this.config.id > 12) player.dimensionBoosts += 1;
        if (this.config.id > 13) player.dimensionBoosts += 1;
        if (this.config.id > 14) player.dimensionBoosts += 1;
        if (this.config.id > 15) {
...
class InfinityIPMultUpgrade extends GameMechanicState {
  ...
  get isRequirementSatisfied() {
    return Achievement(41).canBeApplied;
  }
