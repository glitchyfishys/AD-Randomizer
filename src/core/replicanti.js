...
function addReplicantiGalaxies(newGalaxies) {
  if (newGalaxies > 0) {
    ...
    const keepResources = Pelle.isDoomed
      ...
      : EternityMilestone.all[20].isReached;
    if (!keepResources) {
...
export function replicantiGalaxy(auto) {
  ...
  Replicanti.amount = Achievement(126).canBeApplied && !Pelle.isDoomed
...
export function getReplicantiInterval(overCapOverride, intervalIn) {
  ...
  if ((TimeStudy(133).isBought && !Achievement(138).canBeApplied) || overCap) {
...
export function totalReplicantiSpeedMult(overCap) {
  let totalMult = DC.D1.mul(Archipelago.replicanti);
...
export const ReplicantiUpgrade = {
  chance: new class ReplicantiChanceUpgrade extends ReplicantiUpgradeState {
    ...
    get autobuyerMilestone() {
      return EternityMilestone.all[21];
    }
  ...
  interval: new class ReplicantiIntervalUpgrade extends ReplicantiUpgradeState {
    ...
    get autobuyerMilestone() {
      return EternityMilestone.all[22];
    }
  ...
  galaxies: new class ReplicantiGalaxiesUpgrade extends ReplicantiUpgradeState {
    ...
    get autobuyerMilestone() {
      return EternityMilestone.all[23];
    }
...
export const Replicanti = {
  ...
  reset(force = false) {
    const unlocked = force ? false : EternityMilestone.all[9].isReached;
    player.replicanti = {
      unl: unlocked,
  ...
  get isUncapped() {
    return (TimeStudy(192).isBought && !Enslaved.isRunning) || PelleRifts.vacuum.milestones[1].canBeApplied;
  }
};
