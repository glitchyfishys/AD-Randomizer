...
export const alchemyResources = {
  ...
  "synergism": {
    ...
    effect: amount => {
      ...
      return Achievement(175).canBeApplied ? rawValue : Math.min(rawValue, 1);
    },
    tier: 3,
    uiOrder: 2,
    unlockedAt: 13,
    ...
    formatEffect(value) {
      return `Alchemy Reaction efficiency ${formatPercents(0.3)} ➜ ${formatPercents(value, 2, 2)}
        ${(!Achievement(175).canBeApplied && value >= 1) ? " (Capped)" : ""}`;
    },
    reagents: [
      {
  ...
  "momentum": {
    ...
    formatEffect: value => `All Dimensions ${formatPow(Ra.momentumValue, 4, 4)}, increasing by
      ${format(0.5 * Achievement(175).effectOrDefault(1), 3, 3)}
