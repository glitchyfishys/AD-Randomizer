...
export class BigCrunchAutobuyerState extends UpgradeableAutobuyerState {
  ...
  get canBeUpgraded() {
    return NormalChallenges.all[11].isCompleted;
  }
  ...
  get hasAdditionalModes() {
    return EternityMilestone.all[4].isReached;
  }
  ...
  autoInfinitiesAvailable(considerMilestoneReached) {
    return (considerMilestoneReached || EternityMilestone.all[25].isReached) &&
  ...
  reset() {
    super.reset();
    if (EternityMilestone.all[4].isReached) return;
