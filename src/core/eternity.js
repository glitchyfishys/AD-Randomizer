...
export function eternity(force, auto, specialConditions = {}) {
  ...
  if (!EternityMilestone.all[1].isReached && !(Pelle.isDoomed && PelleUpgrade.keepAutobuyers.canBeApplied)) {
...
export function initializeResourcesAfterEternity() {
  ...
  player.dimensionBoosts = (EternityMilestone.all[3].isReached) ? 4 : 0;
  player.galaxies = (EternityMilestone.all[3].isReached) ? 1 : 0;
...
export function gainedEternities() {
  return Pelle.isDisabled("eternityMults")
    ? new Decimal(3)
    : new Decimal(getAdjustedGlyphEffect("timeetermult")).mul(Archipelago.eternities)
