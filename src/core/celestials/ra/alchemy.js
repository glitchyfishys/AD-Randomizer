...
class AlchemyReaction {
  ...
  get actualYield() {
    ...
    for (const reagent of this._reagents) {
      ...
      const reagentAfter = reagent.resource.amount - Yield * reagent.cost;
      const diffBefore = reagentBefore - prodBefore;
      const diffAfter = reagentAfter - prodAfter;
      cappedYield = Math.min(cappedYield, Yield * diffBefore / (diffBefore - diffAfter));
    }
  ...
  get priority() {
    ...
    for (const reagent of this._reagents) {
      const afterReaction = reagent.resource.amount - reagent.cost * Yield;
