...
export const Pelle = {
  ...
  get specialGlyphEffect() {
    ...
    return {
      ...
      replication: isActive("replication")
        ? 10 ** 54 ** (PelleRifts.vacuum.percentage)
        : 1,
    ...
    switch (type) {
      ...
      case "time":
        return `Eternity Point gain ${formatX(Currency.eternityPoints.value.plus(1).pow(0.3), 2)}
          (based on current EP)`;
      case "replication":
        return `Replication speed ${formatX(10 ** 54 ** (PelleRifts.vacuum.percentage), 2)} \
  ...
  get remnantsGain() {
    ...
    const gain = ( (Math.log10(am + 2) + Math.log10(ip + 2) + Math.log10(ep + 2)) / 1.64) ** 7.5 * Archipelago.remnants;

    return gain < 1 ? gain : Math.floor(gain - this.cel.remnants);
  },

  realityShardGain(remnants) {
    return Decimal.pow(10, remnants ** (1 / 7.5) * 4).minus(1).div(1e3).mul(Archipelago.realityShards);
  },
