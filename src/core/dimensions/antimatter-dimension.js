...
export function antimatterDimensionCommonMultiplier() {
  ...
  if (Pelle.isDoomed) multiplier = multiplier.dividedBy(1e5);
...
export function getDimensionFinalMultiplierUncached(tier) {
  ...
  multiplier = multiplier.timesEffectOf(DilationUpgrade[5]);
...
function applyNDMultipliers(mult, tier) {
  ...
  if (tier === 1) {
    multiplier = multiplier
      .timesEffectsOf(
        InfinityUpgrade[9],
        InfinityUpgrade[9].chargedEffect,
        Achievement(28),
        Achievement(31),
        Achievement(68),
        Achievement(71),
...
function applyNDPowers(mult, tier) {
  ...
  multiplier = multiplier
    .powEffectsOf(
      ...
      InfinityUpgrade[0].chargedEffect,
      InfinityUpgrade[8].chargedEffect,
      AlchemyResource.power,
      Achievement(183),
      PelleRifts.paradox
    ).pow(Archipelago.antimatter);
...
class AntimatterDimensionState extends DimensionState {
  ...
  get infinityUpgrade() {
    switch (this.tier) {
      case 1:
      case 8:
        return InfinityUpgrade[1];
      case 2:
      case 7:
        return InfinityUpgrade[3];
      case 3:
      case 6:
        return InfinityUpgrade[2];
      case 4:
      case 5:
        return InfinityUpgrade[6];
    }
    return false;
  }
  ...
  get isAvailableForPurchase() {
    if (!EternityMilestone.all[19].isReached && this.tier > DimBoost.totalBoosts + 4) return false;
    const hasPrevTier = this.tier === 1 || AntimatterDimension(this.tier - 1).totalAmount.gt(0);
    if (!EternityMilestone.all[19].isReached && !hasPrevTier) return false;
...
export const AntimatterDimensions = {
  ...
  get buyTenMultiplier() {
    ...
    mult = mult.pow(getAdjustedGlyphEffect("effarigforgotten")).powEffectOf(InfinityUpgrade[4].chargedEffect);
