...
function galaxyReset() {
  ...
  player.galaxies++;
  if (!Achievement(143).canBeApplied || (Pelle.isDoomed && !PelleUpgrade.galaxyNoResetDimboost.canBeApplied)) {
...
export function manualRequestGalaxyReset(bulk) {
  ...
  if (player.options.confirmations.antimatterGalaxy) {
    Modal.antimatterGalaxy.show({ bulk: bulk && EternityMilestone.all[8].isReached });
    return;
  }
...
export function requestGalaxyReset(bulk, limit = Number.MAX_VALUE) {
  ...
  if (EternityMilestone.all[8].isReached && bulk) return maxBuyGalaxies(restrictedLimit);
