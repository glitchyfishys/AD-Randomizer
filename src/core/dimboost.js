...
export class DimBoost {
  static get power() {
    ...
    let boost = Effects.max(
      ...
      ).powEffectsOf(InfinityUpgrade[10].chargedEffect);
  ...
  static bulkRequirement(bulk) {
    ...
    amount -= Effects.sum(InfinityUpgrade[5]);
    if (InfinityChallenge(5).isCompleted) amount -= 1;

    amount *= InfinityUpgrade[5].chargedEffect.effectOrDefault(1);
  ...
  static get unlockedByBoost() {
    ...
    const allNDUnlocked = EternityMilestone.all[19].isReached;
  ...
  static get startingDimensionBoosts() {
    let i = 0
    if (InfinityUpgrade[15].isBought) i += 1;
    if (InfinityUpgrade[14].isBought) i += 1;
    if (InfinityUpgrade[13].isBought) i += 1;
    if (InfinityUpgrade[12].isBought) i += 1;
    return i;
  }
}
...
export function softReset(tempBulk, forcedADReset = false, forcedAMReset = false, enteringAntimatterChallenge = false) {
  ...
  const canKeepAntimatter = Pelle.isDoomed
    ? PelleUpgrade.dimBoostResetsNothing.canBeApplied
    : (Achievement(111).canBeApplied || Perk.antimatterNoReset.canBeApplied);
...
export function skipResetsIfPossible(enteringAntimatterChallenge) {
  ...
  let i = 0;
  if (InfinityUpgrade[15].isBought) {
    i += 1;
  ...
  player.dimensionBoosts = Math.max(player.dimensionBoosts, i);
}
...
export function requestDimensionBoost(bulk) {
  ...
  if (BreakInfinityUpgrade[10].isBought && bulk) maxBuyDimBoosts();
  else softReset(1);
}
