...
NormalTimeStudies.pathList = [
  { path: TIME_STUDY_PATH.ANTIMATTER_DIM, studies: [71, 81, 91, 101], name: "Split 1" },
  { path: TIME_STUDY_PATH.INFINITY_DIM, studies: [72, 82, 92, 102], name: "Split 2" },
  { path: TIME_STUDY_PATH.TIME_DIM, studies: [73, 83, 93, 103], name: "Split 3" },
...
export class NormalTimeStudyState extends TimeStudyState {
  ...
  costsST() {
    return this.config.requiresST && this.config.requiresST.some(s => TimeStudyWithIds(s).isBought);
  }

  checkRequirement() {
    const check = req => (typeof req === "number"
      ? TimeStudyWithIds(req).isBought
      : req());
  ...
  purchaseUntil() {
    TimeStudyTree.commitToGameState(buyStudiesUntil(this._tID));
  }
...
NormalTimeStudyState.allWithIds = Array.from(NormalTimeStudyState.studies);

/**
 * @returns {NormalTimeStudyState}
 */
export function TimeStudyWithIds(id) {
  return NormalTimeStudyState.allWithIds[id];
}
