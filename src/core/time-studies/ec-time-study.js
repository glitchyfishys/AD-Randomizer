import { TimeStudy, TimeStudyWithIds } from "./normal-time-study";
...
export class ECTimeStudyState extends TimeStudyState {
  ...
  purchaseUntil() {
    ...
    if (this.id === 11) {
      TimeStudyWithIds(191).purchase();
    } else if (this.id === 12) {
      TimeStudyWithIds(193).purchase();
    }
    this.purchase();
  }
  ...
  get canBeBought() {
    ...
    if (!this.config.requirement.some(s => TimeStudyWithIds(s).isBought)) {
      return false;
    }
  ...
  get hasForbiddenStudies() {
    return this.config.secondary.forbiddenStudies?.some(s => TimeStudyWithIds(s).isBought);
  }
