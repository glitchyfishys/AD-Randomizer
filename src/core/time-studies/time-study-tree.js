...
export class TimeStudyTree {
  ...
  static commitToGameState(studyArray, auto = true, startEC = false) {
    for (const item of studyArray) {
      const study = typeof item === "number" ? TimeStudyWithIds(item) : item;
  ...
  attemptBuyArray(studyArray, checkCosts) {
    ...
    // console.log(this.purchasedStudies)
  }
  ...
  hasRequirements(study, checkOnlyStructure = false) {
    ...
    const check = req => (typeof req === "number"
      ? this.purchasedStudies.some(x => x._tID == req)
      : req());
    ...
    switch (config.reqType) {
      case TS_REQUIREMENT_TYPE.AT_LEAST_ONE:
        ...
        // console.log(study, config.id, config.requirement.some(r => check(r)))
        break;
      case TS_REQUIREMENT_TYPE.ALL:
        reqSatisfied = config.requirement.every(r => check(r));
        // console.log(study, config.requirement, config.requirement.every(r => check(r)))
        break;
      case TS_REQUIREMENT_TYPE.DIMENSION_PATH:
        ...
        // console.log(study, config.requirement, config.requirement.some(r => check(r)), this.currDimPathCount, this.allowedDimPathCount)
        break;
      default:
  ...
  buySingleStudy(study, checkCosts) {
    ...
    const stNeeded = config.STCost && config.requiresST.some(s => this.purchasedStudies.includes(s))
  ...
  get currDimPathCount() {
    return [TimeStudyWithIds(71).id, TimeStudyWithIds(72).id, TimeStudyWithIds(73).id]
    .countWhere(x => this.purchasedStudies.some(z => z.id == x));
  }

  get allowedDimPathCount() {
    if (DilationUpgrade[7].isBought) return 3;
    if (TimeStudy(201).isBought && !Enslaved.isRunning) return 2;
    return 1;
  }
