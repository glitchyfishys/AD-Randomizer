...
export function getTickSpeedMultiplier() {
  ...
  const effects = Effects.product(
    InfinityUpgrade[7],
    InfinityUpgrade[7].chargedEffect,
    BreakInfinityUpgrade[5],
    TimeStudy(212),
    TimeStudy(232),
    Achievement(86),
    Achievement(178),
...
export const Tickspeed = {
  ...
  get isUnlocked() {
    return AntimatterDimension(2).bought > 0 || EternityMilestone.all[19].isReached ||
  ...
  get current() {
    const tickspeed = Effarig.isRunning
      ? Effarig.tickspeed
      : this.baseValue.powEffectOf(DilationUpgrade[12]);
