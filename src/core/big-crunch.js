...
export function bigCrunchResetValues(enteringAntimatterChallenge) {
  ...
  if (Achievement(95).canBeApplied && !Pelle.isDoomed) {
...
export function preProductionGenerateIP(diff) {
  if (InfinityUpgrade[11].isBought) {
    ...
    let gainedPerGen = player.records.bestInfinity.time >= 999999999999 ? DC.D0 : InfinityUpgrade[11].effectValue;
  ...
  Currency.infinityPoints.add( Decimal.mul(BreakInfinityUpgrade[9].effectOrDefault(0),diff / 60000));
}
