...
export const GameStorage = {
  ...
  get localStorageKey() {
    return DEV ? "RandomDimSaveButForTesting" : "RandomDimSave";
  },

  backupDataKey(saveSlot, backupSlot) {
    return DEV ? `backupRandomTestSave-${saveSlot}-${backupSlot}` : `backupRandomSave-${saveSlot}-${backupSlot}`;
  },

  backupTimeKey(saveSlot) {
    return DEV ? `backupRandomTestTimes-${saveSlot}` : `backupRandomTimes-${saveSlot}`;
  },
...
(${this.exportDateString}).txt`, GameSaveSerializer.serialize(backupData));
  ...
  loadPlayerObject(playerObject) {
    ...
    if (playerObject === Player.defaultStart || checkString !== "") {
      ...
      player = migrations.patch(playerObject);
    }
