...
export function playerInfinityUpgradesOnReset() {
  ...
  const infinityUpgrades = 2 ** 17 - 1;

  const breakInfinityUpgrades = 2 ** 12 - 1;

  if (PelleUpgrade.keepBreakInfinityUpgrades.canBeApplied) return;
  
  if (PelleUpgrade.keepInfinityUpgrades.canBeApplied) {
    player.breakInfinityUpgrades = 0;
  ...
  if (RealityUpgrade(10).isBought || EternityMilestone.all[7].isReached) {
    if (RealityUpgrade(10).isBought || EternityMilestone.all[3].isReached) player.infinityUpgrades = infinityUpgrades;
    else player.infinityUpgrades = 0;
    player.breakInfinityUpgrades = breakInfinityUpgrades;
    player.infinityRebuyables = [8, 7, 10];
  } else if (EternityMilestone.all[3].isReached) {
    player.infinityUpgrades = infinityUpgrades;
    player.breakInfinityUpgrades = 0
    player.infinityRebuyables = [0, 0, 0];
  } else {
    player.infinityUpgrades = 0;
    player.breakInfinityUpgrades = 0
    player.infinityRebuyables = [0, 0, 0];
  }
...
export function gainedInfinityPoints() {
  ...
  ip = ip.pow(Archipelago.infinityPoints);
...
export function gainedEternityPoints() {
  ...
  ep = ep.pow(Archipelago.eternityPoints);
...
export function getOfflineEPGain(ms) {
  if (!EternityMilestone.all[5].isReached || !isOfflineEPGainEnabled()) return DC.D0;
...
export function gainedInfinities() {
  if (EternityChallenge(4).isRunning) return DC.D1;
  if (Pelle.isDisabled("InfinitiedMults")) {
    return DC.D3;
  }
...
export function realTimeMechanics(realDiff) {
  ...
  if (AlchemyResource.momentum.isUnlocked) {
    player.celestials.ra.momentumTime += realDiff * Achievement(175).effectOrDefault(1) * 200;
  }
  ...
  if (player.archipelago.dilationTrapTime > 0){
    player.archipelago.dilationTrapTime -= realDiff;
  }
...
function passivePrestigeGen() {
  ...
  if (!EternityChallenge(4).isRunning) {
    let infGen = DC.D0;
    if (BreakInfinityUpgrade[6].isBought) {
...
function applyAutoUnlockPerks() {
  ...
  if (Perk.autounlockDilation3.canBeApplied) buyDilationUpgrade(DilationUpgrade[9].id);
...
function applyAutoprestige(diff) {
  ...
  if (InfinityUpgrade[11].isCharged) {
    const addedRM = MachineHandler.gainedRealityMachines
      .timesEffectsOf(InfinityUpgrade[11].chargedEffect)
      .times(diff / 1000);
...
function updateTachyonGalaxies() {
  const tachyonGalaxyMult = Effects.max(1, DilationUpgrade[3]);
  ...
  player.dilation.totalTachyonGalaxies *= DilationUpgrade[11].effectValue;
}
...
export function getTTPerSecond() {
  ...
  const dilationTT = DilationUpgrade[9].isBought
    ? DilationUpgrade[9].effectValue.times(Pelle.isDoomed ? 1 : ttMult)
    : DC.D0;
...
export function simulateTime(seconds, real, fast) {
  ...
  if (InfinityUpgrade[16].isBought && player.options.offlineProgress) {
...
export function init() {
  ...
  if (player.archipelago.isArch) Archipelago.Client.login(player.archipelago.lastURL, player.archipelago.lastName, "Antimatter Dimensions Randomizer", {
      ...
      .catch((er) => {
        console.error(er);
        GameUI.notify.error("Did not automatically connect to Archipelago server", 5000);
      });
